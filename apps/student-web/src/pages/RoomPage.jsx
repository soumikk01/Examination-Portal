        ...
        <div style={{ minWidth: 'max-content', padding: '0 20px 10px', pointerEvents: 'none' }}>
          <div style={styles.grid}>
            ...
            {Array.from({ length: 6 }).map((_, r) => (
              <div key={r} style={{ ...styles.deskRow, paddingTop: '15px' }}>
                {skeletonCols.map((_, cIdx) => {
                  const cx = (skeletonCols.length - 1) / 2;
                  const dx = cIdx - cx;
                  return (
                    <div key={cIdx} style={{
                      ...
                      transform: `translateY(${Math.pow(dx, 2) * -1.8}px) rotate(${dx * 1.5}deg)`,
                    }}>
...
const styles = {
  ...
  desk: (isMySeat, isExtra) => ({
    ...
    boxShadow: isMySeat ? '0 6px 16px rgba(59,130,246,0.4)' : '0 2px 4px rgba(0,0,0,0.04)',
    flexShrink: 0,
  }),
  avatarContainer: {
          ...
          >
            <div style={{ minWidth: 'max-content', padding: '4px 10px 10px' }}>
              <div style={styles.grid}>
                ...
                <div style={{ ...styles.deskRow, marginBottom: '8px', alignItems: 'center', paddingTop: '15px' }}>
                  <div style={{ ...styles.desk(false, true), transform: `translateY(${Math.pow(0 - (columns.length - 1)/2, 2) * -1.8}px) rotate(${(0 - (columns.length - 1)/2) * 1.5}deg)` }}>
                    <strong style={{ color: '#94a3b8', fontSize: '9px', letterSpacing: '0.05em' }}>EXTRA</strong>
                  </div>

                  {columns.slice(1, -1).map((col, i) => {
                    const cIdx = i + 1;
                    const cx = (columns.length - 1) / 2;
                    const dx = cIdx - cx;
                    return (
                      <div key={i} style={{
                  ...
                  <div style={{ ...styles.desk(false, false), background: '#1e293b', border: 'none', transform: `translateY(${Math.pow((columns.length - 1) - (columns.length - 1)/2, 2) * -1.8}px) rotate(${((columns.length - 1) - (columns.length - 1)/2) * 1.5}deg)` }}>
